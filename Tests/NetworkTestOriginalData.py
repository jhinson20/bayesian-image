...
#Variable decides if the network is trained on all data in the file or if it is only trained on 8 of the rows at a time
trainAllData = False
...
data = dataFile.sample(n=sampleSize, random_state=seed, replace=False)
...
for foldNum, (train_index, test_index) in enumerate(kf.split(data), 1):
    ...
    if trainAllData:
        #Fit data to network
        graph.fit(data, state_names={'shape': [0, 1], 'x0': [0, 1], 'x1': [0, 1], 'x2': [0, 1], 'x3': [0, 1]}, 
